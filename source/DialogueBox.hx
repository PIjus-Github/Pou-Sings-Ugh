...
{
	var box:FlxSprite;
	var audio:flixel.system.FlxSound = new flixel.system.FlxSound();
	var contador = 1;
	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		...
		switch (PlayState.SONG.song.toLowerCase())
		{
			...
			case 'thorns':
				...
				FlxG.sound.music.fadeIn(1, 0, 0.8);
			case 'sky-jump' | 'moldy-drops' | 'six-ft-under':
		...
		switch (PlayState.SONG.song.toLowerCase()){
			...
			case 'sky-jump' | 'moldy-drops' | 'six-ft-under':
				...
				for (ar in bgJson){
					...
					bg = new FlxSprite(ar[1], ar[2]).loadGraphic(Paths.image("fondos/" + ar[0]));
					bg.scrollFactor.set();
				...
				portraitLeft.frames = Paths.getSparrowAtlas('escenario/beatPortrait');
				portraitLeft.animation.addByPrefix('enter', 'Senpai Portrait Enter', 24, false);
				...
				portraitRight.frames = Paths.getSparrowAtlas('escenario/bfPortrait');
				portraitRight.animation.addByPrefix('enter', 'Boyfriend portrait enter', 24, false);
				...
				portraitGF.frames = Paths.getSparrowAtlas('escenario/gfPortrait2');
		...
		dropText.color = colores[0];
		add(dropText);

		swagDialogue = new FlxTypeText(240, 500, Std.int(FlxG.width * 0.6), "", 32);
		swagDialogue.font = 'Pixel Arial 11 Bold';
		swagDialogue.color = colores[1];
	...
	function startDialogue():Void
	{
		...
		switch(PlayState.SONG.song.toLowerCase()){
			case "sky-jump":
				checar = 1;
				if(contador <= 3){
					audio.loadEmbedded(Paths.sound('tuto/audio' + contador));
					audio.play();
					contador++;
